...
void rook(int a1, int a2, int b1, int b2) { // 6 куда может пойти ладья
...
void sort3(int a, int b, int c) { // 13 сортировка трех чисел в порядке возрастания
	if (a <= b && a <= c) {
		if (b <= c) cout << a << " " << b << " " << c;
		else cout << a << " " << c << " " << b;
	}
